...
def get_text(message: Message) -> new_func2():
...
def get_user(message: Message, text: str) -> new_func3():
    if text is None:
        asplit = None
    else:
        asplit = text.split(" ", 1)
...
async def ytmusic(client, message: Message):
    ...
    if not urlissed:
        await pablo.edit("Yanlış komanda,Daha çox məlumat üçün Kömək Menyusunu yoxlayın")
        return
