...
async def hfmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        lel = await message.reply("`İşlənir ...`")
        if not message.chat.id in DISABLED_GROUPS:
            await lel.edit("Bu Çatda Artıq Aktivləşdirilmiş Musiqi Oxunur")
            return
        DISABLED_GROUPS.remove(message.chat.id)
        await lel.edit(
            f"Söhbətdəki istifadəçilər üçün musiqi pleyeri uğurla aktivləşdirildi {message.chat.id}"
        )
        ...
        await lel.edit(
            f"Söhbətdəki istifadəçilər üçün musiqi pleyeri uğurla deaktiv edildi {message.chat.id}"
        )
    else:
        await message.reply_text(
            "I only recognize `/musicplayer on` and /musicplayer `off only`"
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if chet_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chet_id)
            await cb.message.edit("Söhbəti uğurla tərk etdiniz!")
        else:
            await cb.answer("Söhbət bağlanmayıb!", show_alert=True)
...
async def play(_, message: Message):
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "Səslidə musiqi oxutmaq üçün bu qrupa qoşuldum"
                    )
                    await lel.edit(
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            message.reply("Qrup zəngi bağlı deyil və ya qoşula bilmirəm")
            return
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in DISABLED_GROUPS:
        return    
    lel = await message_.reply("🔄 **İşlənir...**")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
    try:
        user = await USER.get_me()
    except:
        user.first_name = "AmourMusicBot"
    usar = user
    wew = usar.id
    try:
        # chatdetails = await USER.get_chat(chid)
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message_.chat.id, "Səslidə musiqi oynamaq üçün bu qrupa qoşuldum"
                    )
                    await lel.edit(
                        "<b>helper userbot joined your chat</b>",
                    )
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        ...
        return
    await cb.message.edit("Musiqi yüklənir... Gözləyin")
    x=int(x)
    try:
