...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        callsmusic.pytgcalls.active_calls[chat_id] == "dayandı!"
    ):
        await message.reply_text("❗ Heç bir şey oxumur!")
    else:
        callsmusic.pytgcalls.pause_stream(chat_id)
        await message.reply_text("▶️ Dayandı!")
...
async def resume(_, message: Message):
    ...
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        ...
        await message.reply_text("⏸ Yenidən davam etdirildi!")
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❗ Heç bir şey mahnı oxumur!")
    else:
        try:
...
async def skip(_, message: Message):
    ...
    chat_id = get_chat_id(message.chat)
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❗ Keçmək üçün heç bir mahnı oynamır!")
    else:
    ...
    if not qeue:
        return
    await message.reply_text(f"- Keçildi **{skip[0]}**\n- Oxuyur **{qeue[0][0]}**")
